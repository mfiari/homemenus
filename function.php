...
function getDistance ($adresse1, $adresse2) {
	...
	if (!isset($details['rows'][0]['elements'][0])) {
		return array(
...
function carteDeFidelite ($request, $commande) {
...
function getContratRestaurant ($restaurant) {
	$file = ROOT_PATH.'files/contrats/contrat'.$restaurant->id.'.pdf';

	if (file_exists($file)) {
		header('Content-Description: File Transfer');
		header('Content-Type: application/octet-stream');
		header('Content-Disposition: attachment; filename="contrat-HoMe_Menus-'.cleanString($restaurant->nom).'.pdf"');
		header('Expires: 0');
