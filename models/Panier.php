...
class Model_Panier extends Model_Template {
	...
	public function getNbArticle () {
		$sql = "SELECT COUNT(pm.id) + COUNT(pc.id) AS total 
		FROM panier
		LEFT JOIN panier_menu pm ON pm.id_panier = panier.id
		LEFT JOIN panier_carte pc ON pc.id_panier = panier.id
	...
	public function validate($rue, $ville, $code_postal, $telephone, $heure_souhaite = -1, $minute_souhaite = 0) {
		$sql = "UPDATE panier SET rue = :rue, ville = :ville, code_postal = :code_postal, telephone = :telephone, heure_souhaite = :heure_souhaite, minute_souhaite = :minute_souhaite";
		$stmt = $this->db->prepare($sql);
